...
describe("NFT main get all token method test", () => {
  ...
  it("should revert the verification if token_id does not belongs to owner", async () => {
  ...
  it("should revert the verification if message is changed (price is changed)", async () => {
    const hashedMessage = await EthCrypto.hash.keccak256(message);
    const signedMessage = await EthCrypto.sign(privateKey1, hashedMessage);
    modifiedObj = { tokenId: 1, price: 2000, message: "Verify signature" };
