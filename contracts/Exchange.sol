...
contract Exchange {
    ...
    function verifySignature(string memory message, bytes memory signature,address contractAddress,uint tokenId)
        public
        view
    {
        address signer =  ECDSA.recover(keccak256(abi.encodePacked(message)), signature);
        require(ERC721(contractAddress).ownerOf(tokenId) == signer,
            "Invalid signature");
    }
}
